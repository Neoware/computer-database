...
<ul class="pagination">
	<c:if test="${page > 1}" >
	<li><mylib:link page="${page - 1}" limit="${limit}"><span aria-hidden="true">&laquo;</span></mylib:link></li>
	</c:if>
	...
	<c:forEach begin="${page}" end="${endPage}" varStatus="loop">
		<c:if test="${loop.index == page}">
		 <li class="active"><mylib:link page="${loop.index}" limit="${limit}">${loop.index}</mylib:link></li>
		</c:if>
		<c:if test="${loop.index != page}">
	...
	<li><mylib:link page="${page + 1}" limit="${limit}"><span aria-hidden="true">&raquo;</span></mylib:link></li>
	</c:if>
</ul>
