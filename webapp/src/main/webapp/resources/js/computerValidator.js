$(document).ready(function() {
	$( ".datepicker" ).datepicker({
		  dateFormat: "dd-mm-yy"
		});
	
	$("#submitButton").click(function(event){
		var validation = {message: "<strong>Error:</strong><br>", success: true};
		var trimmedName = jQuery.trim($('#computerName').val());
  		var trimmedCompany = jQuery.trim($('#companyId option:selected').text());
  		var trimmedIntroduced = jQuery.trim($('#introduced').val());
  		var trimmedDiscontinued = jQuery.trim($('#discontinued').val());
  		var introducedDate;
  		var discontinuedDate;
  		checkName(trimmedName, validation);
  		checkCompany(trimmedCompany, validation);
  			
  		if (checkDate(trimmedIntroduced, validation) == false){
  			validation.message += "Invalid introduced date<br>";
  		}
  		else if (trimmedIntroduced.length > 0){
  			introducedDate = $("#introduced").datepicker("getDate");
  		}
  		if (checkDate(trimmedDiscontinued, validation) == false){
  			validation.message += "Invalid discontinued date<br>";
  		}
  		else if (trimmedDiscontinued.length > 0){
  			discontinuedDate = $("#discontinued").datepicker("getDate");
  		}
 
  		
  		if ( typeof introducedDate !== 'undefined' && typeof discontinuedDate !== "undefined" ) {
  			if (introducedDate.getTime() > discontinuedDate.getTime()){
  				validation.success = false;
  				validation.message += "Introduced date is after discontinued date<br>";
  			}
  		}
  		if (validation.success === false){
  			$( "#information" ).html( "<div class='alert alert-danger' role='alert'>" + validation.message + "</div>" );
  	  			event.preventDefault();
  		}
});
});

function checkName(trimmedName, validation) {
	if (trimmedName.length == 0)  {
  			validation.success = false;
  			validation.message += "Name is mandatory<br>";
  		}
  		if (trimmedName.length > 255){
  			validation.success = false;
  			validation.message += "Computer name must be less than 255 characters<br>";
  		}
}

function checkCompany(trimmedCompany, validation) {
	if (trimmedCompany.length > 255){
  			validation.success = false;
  			validation.message += "Company name must be less than 255 characters<br>";
  		}
}

function checkDate(dateString, validation) {
var pattern = new RegExp("^(?:(?:31(-)(?:0?[13578]|1[02]))\\1|(?:(?:29|30)(-)(?:0?[1,3-9]|1[0-2])\\2))(?:(?:1[6-9]|[2-9]\\d)?\\d{2})$|^(?:29(-)0?2\\3(?:(?:(?:1[6-9]|[2-9]\\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\\d|2[0-8])(-)(?:(?:0?[1-9])|(?:1[0-2]))\\4(?:(?:1[6-9]|[2-9]\\d)?\\d{2})$");
if (dateString.length > 0 && !pattern.test(dateString)){
  			validation.success = false;
  			return false;
  		}
	return true;
}



